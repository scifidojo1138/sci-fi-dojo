...
- **EMAIL ME MY LINK** (2026-07-13) lives in the same lookup card under FIND MY ACCOUNT: `submitSendLink()` posts the typed email to `send_link`, which mails the account link to the on-file address (never the requester's input as a destination beyond matching). The app always shows the same "If that email is on an account..." confirmation, mirroring the backend's identical-either-way response. Signup also emails the link automatically (`link_emailed` drives a "We also emailed it to you" line on the success screen), and signup now requires **both** email and phone client-side and server-side.
