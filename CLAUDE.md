...
- **"Send me my account link" recovery** (unstaffed-store review, 2026-07-12): signup requires only email OR phone, but `customer_lookup` requires BOTH — an email-only signup that loses its bookmark is locked out with no staff on site. Plan: Apps Script MailApp emails the token link to the on-file address (inbox possession = proof); also consider sending the link automatically at signup, which makes lost bookmarks mostly a non-event.
