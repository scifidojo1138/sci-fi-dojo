...
- **News & Updates is one collapsed section**, not three always-expanded ones: `#newsUpdatesWrap` reuses the generic `toggleNote('NewsUpdates')` accordion (same mechanism as the login screen's email/phone lookup) around the existing `#rewindSection`/`#eventsSection`/`#perksSection` containers, which are otherwise unchanged. `renderFeeds()` computes a combined badge count and hides the wrapper entirely when all three are empty.
