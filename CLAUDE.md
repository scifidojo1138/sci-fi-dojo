...
- **Where it's redeemed in the app**: a single collapsible "Have a promo code?" card on the Rent screen (`#screen-code`) — deliberately NOT on the signup form and NOT a separate Account-screen field, since every customer (brand new right after signup, or returning) passes through this exact screen every time they're about to rent. `goCode()` hides the card entirely once `member.promo_redeemed` is true and renders a "Your next rental is free!" banner when `member.free_rental_credits > 0`.
...
- **Browse is discovery-only** (no RENT button — renting starts by scanning the disc in hand at the cabinet). Rows tap-to-expand (`browseExpanded` map lives outside the render so star-toggle re-renders keep rows open) into a detail panel: cover art loaded **on demand from `/covers/<item_id>.jpg`** (repo-committed by filename convention, no sheet column — see `/covers/README.md`; the `<img>` element only exists while a row is open, so covers never preload, and `coverMissing` suppresses re-requests for 404'd images), a price line (`rental_price` rides the `catalog` payload since the 2026-07-08 backend; the line is hidden when the field is absent so an old deploy never shows a guessed price), a maximum-charge fact (`cap_cents`, added 2026-07-13 — the backend's resolved `payoffCapCents_` value, same hide-if-absent rule as the price line), subcategory/group/disc-count/condition facts (defaults `standard`/`good` are hidden as noise), and an availability line.
...
- **Promo code redemption lives on the Rent screen, one field for everyone** (2026-07-14): a collapsible "Have a promo code?" card sits right on `#screen-code`, same `toggleNote()` accordion mechanism as Hours & Location — deliberately not on the signup form and not a separate Account-screen field, since new customers land on this exact screen right after signup (`goFirstRental()`) and returning customers land here every time they tap RENT. `submitPromoCode()` posts to `redeem_promo`, then re-fetches (`refreshAfterRent()`) and re-renders (`goCode()`) rather than trusting the response shape. `goCode()` hides the card entirely once `member.promo_redeemed` and shows a "Your next rental is free!" banner when `member.free_rental_credits > 0`. `lookupItem()`'s price line, the PAY & RENT button text, and the success receipt all branch on the credit too ("Free!" / "CONFIRM FREE RENTAL" / "Free (promo credit)") — the free/normal branching happens in three places, not one shared helper, since each has different surrounding copy.
...
- **Dashboard is trim, four tiles only** (decision 2026-07-10): RENT, RETURN, BROWSE, ACCOUNT. The old six-tile grid (RENT/RETURN/MY RENTALS/ACCESS CABINET/BROWSE/ACCOUNT), the account-status stat grid, the per-rental running-cost cards, and the Return Bin notification banner are all gone from the home screen — moved into Account (status detail, My Rentals) or the Return screen (bin visibility), or folded into a tile subtitle (RENT shows "N available" / "Limit reached" directly, computed the same way `backendGetMemberStatus()` always has). `renderCurrentRentals()`/`statCardFull()` are left defined but unused (harmless — `getElementById` on a removed id just no-ops) rather than deleted, in case the running-cost card is wanted back later.
...
- **Self-service card update** (unstaffed-store review; fully written 2026-07-13, deploy pending): rent.html UI (live), the backend `rent_card_lookup`/`rent_card_updated` actions, the Netlify `update-card` function, and the extended `rental-webhook` (setup-mode session → new card becomes invoice default, old cards detached → `rent_card_updated`) are all written and unit-tested. Remaining: deploy the backend, commit `update-card.js` + updated `rental-webhook.js` to the onboarding repo, test in Stripe test mode (card `4000 0000 0000 0341` to force a failure, then update to `4242...`). No Stripe dashboard change needed — setup sessions fire the same `checkout.session.completed` event. Owed charges are NOT auto-retried on card update — the next billing sweep (or CHARGE & CLOSE at return) collects.
