...
- **Self-service card update** (unstaffed-store review; built 2026-07-13, one piece left): rent.html UI, the backend `rent_card_lookup`/`rent_card_updated` actions, and the Netlify `update-card` function are all written and tested. Remaining: extend the onboarding repo's `rental-webhook` function to handle `checkout.session.completed` for **setup-mode** sessions (set the new card as the customer's payment method for future charges, then POST `rent_card_updated`), deploy the backend + functions, add the setup event if the Stripe webhook filters by mode. Owed charges are NOT auto-retried on card update — the next billing sweep (or CHARGE & CLOSE at return) collects.
