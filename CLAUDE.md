...
- **RENT leads to the cabinet code screen first, item entry second** — `goRentFlow()` (the tile's onclick) just calls `goCode()`; the code screen gained a CONTINUE TO RENT A DISC button that calls `goCheckout()`. The code needs no gating beyond the existing `active` check since it's handed out at signup regardless of any rental. `goFirstRental()` (the post-signup success-screen CTA) follows the same path once the account warm-up resolves. Back-navigation from any screen still jumps straight to the dashboard (no back-stack) — a deliberate simplicity tradeoff, not an oversight.
...
- **The "Free Account" pill** (`.member-tier.tier-mat` markup) moved from the home header to the top of the Account screen; the CSS classes themselves are generic (border+dot+text pill), not membership-tier-specific, so they're reused as-is rather than renamed.
