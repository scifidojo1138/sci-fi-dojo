...
- **Fix customer_id generation BEFORE go-live** (decision 2026-07-13, deferred until the pre-launch test-account wipe): ids come from `'CUS-' + sheet.getLastRow()`, so deleting rows makes new signups reuse existing ids — this happened live in testing (two CUS-005 accounts; the second inherited the first's open rental and couldn't rent). Fix: mint from the max existing `CUS-###` suffix across **Customers AND Rentals** (+1), wrapped in `LockService` (the row-count approach also races concurrent signups). Do NOT switch the Rentals tab to `customer_token` instead — tokens are bearer secrets and should stay out of extra tabs/payloads, and the collision would persist in Transactions/Catalog anyway. The pre-launch wipe must clear Customers + Rentals + rental Transactions **together**, or fresh ids will collide with leftover history. Post-launch habit: mark accounts `status: closed` rather than deleting rows.
